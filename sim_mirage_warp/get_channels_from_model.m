function [channels, rays, is_ray_blocked]  = get_channels_from_model(model,src,dst,enable_printing,offset)
    ...
    for i=1:length(rays)
        ...
        for j=1:length(all_blocking_rays)
            if(sum(abs(ray_status(i,:)-blocking_ray_status(j,:)))==0)
                is_ray_clear(i) = model.obs_attenuation;
%                 if(enable_printing)
...
%                 channels(j) = channels(j) + is_ray_clear(i)*amps(j)*1/(cur_d/lambda(j))*exp(-1j*2*pi/lambda(j)*(cur_d+offset));% * src_atten*dst_atten;
                channels(j) = channels(j) + exp(-1j*2*pi/lambda(j)*(cur_d+offset));% * src_atten*dst_atten;
